...
func TestLoadAcceptsProductionDemoOrbitWithoutAPIKey(t *testing.T) {
	clearReleaseEnvironment(t)
	t.Setenv("APP_ENV", "production")
	t.Setenv("DEV_LOGIN_ENABLED", "false")
	t.Setenv("PUBLIC_BASE_URL", "https://api.jianwo.example")
	...
	t.Setenv("AI_ROUTING_JSON", `{
		"models":{
			"qwen":{"vendor":"aliyun","protocol":"openai_chat_completions","model":"qwen","base_url":"https://ai.example.com/v1","api_key_env":"ALIYUN_API_KEY"},
			"image":{"vendor":"aliyun","protocol":"dashscope_wan","model":"wan","base_url":"https://images.example.com/generate","api_key_env":"ALIYUN_API_KEY"},
			"demo/body-orbit":{"vendor":"demo","protocol":"demo_orbit","model":"body-orbit","base_url":"https://127.0.0.1/demo-orbit","api_key_env":"AI_TEST_PRIMARY_KEY"}
		},
		"routes":{
			"appearance_analysis":{"primary":"qwen"},
			"outfit_diagnosis":{"primary":"qwen"},
			...
			"today_plan":{"primary":"qwen"},
			"hair_edit":{"primary":"image"},
			"body_orbit":{"primary":"demo/body-orbit"}
		}
	}`)
	...
	if cfg.AIRouting.Routes["body_orbit"].Primary != "demo/body-orbit" {
		t.Fatalf("body_orbit route=%#v", cfg.AIRouting.Routes["body_orbit"])
	}
}

func TestLoadRejectsNonOrbitDemoInProduction(t *testing.T) {
	clearReleaseEnvironment(t)
	t.Setenv("APP_ENV", "production")
	t.Setenv("DEV_LOGIN_ENABLED", "false")
	t.Setenv("PUBLIC_BASE_URL", "https://api.jianwo.example")
	...
	t.Setenv("AI_ROUTING_JSON", `{
		"models":{
			"qwen":{"vendor":"aliyun","protocol":"openai_chat_completions","model":"qwen","base_url":"https://ai.example.com/v1","api_key_env":"ALIYUN_API_KEY"},
			"image":{"vendor":"aliyun","protocol":"dashscope_wan","model":"wan","base_url":"https://images.example.com/generate","api_key_env":"ALIYUN_API_KEY"},
			"demo/advisor":{"vendor":"demo","protocol":"openai_chat_completions","model":"demo","base_url":"https://ai.example.com/v1","api_key_env":"ALIYUN_API_KEY"}
		},
		"routes":{
			"appearance_analysis":{"primary":"qwen"},
			"outfit_diagnosis":{"primary":"qwen"},
			"purchase_diagnosis":{"primary":"qwen"},
			"advisor_chat":{"primary":"demo/advisor"},
			"today_plan":{"primary":"qwen"},
			"hair_edit":{"primary":"image"}
		}
	}`)
	_, err := Load()
	if err == nil || !strings.Contains(err.Error(), "must not use demo vendor") {
		t.Fatalf("expected non-orbit demo vendor to be rejected, got %v", err)
	}
}

func TestProductionAIRoutingConfigIsValid(t *testing.T) {
	t.Setenv("BAILIAN_WORKSPACE_ID", "workspace-test")
	data, err := os.ReadFile("../../config/ai-routing.production.json")
	if err != nil {
		t.Fatal(err)
	}
	var routing AIRoutingConfig
	...
	if routing.Routes["body_orbit"].Primary != "demo/body-orbit" {
		t.Fatalf("production routing missing body_orbit: %#v", routing.Routes["body_orbit"])
	}
}
