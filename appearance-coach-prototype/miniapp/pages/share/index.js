...
Page({
  ...
  savePoster() {
    ...
    query.select('#shareCanvas').fields({ node: true, size: true }).exec((result) => {
      ...
      context.fillStyle = '#587344'; context.font = '18px sans-serif'; context.fillText('见我 · 私人形象顾问', 28, 42)
  ...
  onShareAppMessage() {
    return { title: `${this.data.snapshot ? this.data.snapshot.title : '我的形象方案'}｜见我`, path: `/pages/share/index?token=${this.data.card ? this.data.card.token : ''}` }
  }
})
