// 穿搭诊断会话：同步请求跨页存活——模块级 inflight Promise + storage 草稿。
...
// 存储键 zsm_outfit_session：这只是「这一页在干什么」的会话草稿，服务端
...
export interface OutfitSessionStore {
  read(key: string): string
  write(key: string, value: string): void
}

/** 复访事件：页面据此 setState；测试据此断言时序与分支。 */
export type OutfitResumeEvent =
  | { type: 'hydrate'; draft: OutfitDraft }
  | { type: 'busy'; busy: boolean }
  | { type: 'result'; item: Diagnosis }
  | { type: 'error'; error: unknown }

export interface OutfitSessionDeps {
...
export function shouldAdoptLatest(
  draft: Pick<OutfitDraft, 'startedAt' | 'mediaId'>,
  latest: Diagnosis,
): boolean {
...
export function createOutfitSession(store: OutfitSessionStore, storageKey = 'zsm_outfit_session') {
  ...
  function persist(next: OutfitDraft) {
    memory = next
    store.write(storageKey, JSON.stringify(next))
  }

  function read(): OutfitDraft | null {
    if (memory) return memory
    try {
      const raw = store.read(storageKey)
      if (raw) {
        const parsed = JSON.parse(raw) as OutfitDraft
  ...
  function writeDraft(patch: Partial<OutfitDraft>) {
    persist({ ...emptyDraft(), ...read(), ...patch })
  }

  /** 请求发起前一刻才标 pending：pending + mediaId 表示「有一趟针对这张照片的请求在途」。 */
  function markPending(patch: Partial<OutfitDraft>) {
  ...
  /**
   * 同一时刻只允许一趟诊断请求；inflight 期间的发起一律接管同一个 Promise。
   * mediaId 是这趟请求针对的照片：落定（done/idle）只在草稿仍指向同一照片时写盘——
   * 飞行中用户重选了照片，迟到的旧结论就不许盖掉新草稿（防旧盖新的收口）。
   */
  ...
  /**
   * 复访恢复时序：
   * 1) 草稿水合先上屏（scene/photoPath/已有结论）；
   * 2) 有 inflight 就接管等待，绝不重新发起；
   * 3) pending 草稿：先问服务端 latest（recentEnough && sameMedia 才接管），
   *    查不到或不接管再用同一 mediaId 续跑——中途换照片则放弃上屏与落盘；
   * 4) 其余情况后台校验 latest：只在比草稿新时接管；freshStart（刚点了再来一次/
   *    重选照片）不查，免得服务端旧结论立刻盖回。
   */
  async function resume(
    deps: OutfitSessionDeps,
    emit: (event: OutfitResumeEvent) => void,
    ...
    if (draft?.pending && draft.mediaId) {
      ...
      try {
        ...
        const item = await runDiagnose(draft.mediaId, () =>
          deps.diagnose({ kind: 'outfit', media_id: draft.mediaId, scene: draft.scene || 'daily' }),
        )
        if (adopted(item)) emit({ type: 'result', item })
      } catch (error) {
        emit({ type: 'error', error })
...
export type OutfitSession = ReturnType<typeof createOutfitSession>

/** 页面共享单例：页面模块只装载一次，模块级 inflight 因此跨页存活。 */
let shared: OutfitSession | null = null
export function sharedOutfitSession(store: OutfitSessionStore): OutfitSession {
  if (!shared) shared = createOutfitSession(store)
  return shared
}
