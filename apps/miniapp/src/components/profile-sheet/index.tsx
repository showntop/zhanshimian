...
import { ME_COPY, PROFILE_SETUP_COPY, type UserProfile } from '@zsm/core'
...
function sheetScrollHeight() {
  ...
  return Math.max(240, Math.round(win.windowHeight * 0.85 - chrome))
}
...
export default function ProfileSheet({ profile, onSaved }: ProfileSheetProps) {
  ...
  const [role, setRole] = useState(profile?.role && profile.role !== ME_COPY.unfilled ? profile.role : PROFILE_SETUP_COPY.skip)
  const [budget, setBudget] = useState(
    profile?.budget && profile.budget !== ME_COPY.unfilled ? profile.budget : PROFILE_SETUP_COPY.skip,
  )
  ...
  useEffect(() => {
    ...
    setRole(profile?.role && profile.role !== ME_COPY.unfilled ? profile.role : PROFILE_SETUP_COPY.skip)
    setBudget(profile?.budget && profile.budget !== ME_COPY.unfilled ? profile.budget : PROFILE_SETUP_COPY.skip)
  ...
  const save = async () => {
    ...
    const next: UserProfile = {
      height_cm: height,
      role: role === PROFILE_SETUP_COPY.skip ? ME_COPY.unfilled : role,
      budget: budget === PROFILE_SETUP_COPY.skip ? ME_COPY.unfilled : budget,
    ...
    try {
      const saved = await peripherals.updateMyProfile(next)
  ...
  return (
    <View className="profile-sheet">
      ...
      <View className="profile-sheet__foot">
        <PrimaryButton text={PROFILE_SETUP_COPY.save} loading={busy} onClick={() => void save()} />
      </View>
    </View>
  )
}
