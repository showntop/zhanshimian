...
- Product name: `怎么打扮` (supersedes the earlier `见我`; confirmed by the user 2026-09-04); position it as a private image consultant, not a beauty-scoring tool.
...
- The five approved generated screens were the unique visual source of truth for the formal mini-program: `exec-46e2f522-6e72-461e-acee-8f7422e16b59.png` (home), `exec-45901ba0-3298-42cc-9375-8633a8f7b19c.png` (capture), `exec-7a593fd5-b009-48f7-89a6-a6012fc43266.png` (report), `exec-7dd1c7ee-d76f-421f-86eb-313e889bfe0d.png` (plans), and `exec-1d4392b2-0c2a-4ae0-b7be-6c49fb6a4181.png` (plan detail). Do not reinterpret their information hierarchy when extending the flow. ⚠️ These five files were never committed and are lost (checked 2026-09-04); the surviving in-repo visual reference is `reference-option-2.png` plus the approved direction in this section.
- Data authenticity rules (miniapp, 2026-09-04): never silently fall back to fake content — a failed/empty API result renders an explicit empty or error state, not bundled stock content. `utils/media.js` contracts: `lookImage()` is strict (invalid URL -> `''`); `exampleImage()` is the only way to show bundled model imagery and any such image must carry the `.example-badge` 「风格参考」/「示例」 badge and the `.example-soft` blur treatment (both defined in `app.wxss`); `userImage()` is for user photos and stays empty on failure. Demo content only enters via explicit server endpoints (`/v1/media/demo`), never client-side injection.
- WXML conditional chains must use `wx:elif`, never `wx:else-if` (2026-09-05): the current toolchain silently ignores `wx:else-if`, so such elements render unconditionally (this caused the report page to show success content stacked on top of the error view), and any following `wx:else` then fails compilation with "wx:if not found". Do not "normalize" `wx:elif` to `wx:else-if`.
...
- Mini-program image assets must be PNG or JPEG, not WebP — WebP rendered as blank in the WeChat build and blanked the home comparison card. PNG files under `assets/` are masters ignored by `project.config.json` packing; shipped photos are JPEG (~40-60 KB each). Icons ship as PNG too: `<image>` does not render SVG reliably on older base libraries, so `assets/icons/*.svg` are masters (ignored on packing) and every page references the generated `assets/icons/*.png` (regenerate with `rsvg-convert -w 128 icon.svg -o icon.png`).
