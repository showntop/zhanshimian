...
本仓库是「uplook」的全新 monorepo 实现。`appearance-coach-prototype/` 是**只读参考**（原型视觉稿、旧实现、历史文档），任何改动不得进入该目录。
...
2. **生成图来源真实性**：
   ...
   - 修订（2026-09-17）：发型设计页示例模特已从主流程退役——只保留「无用户照片空态」的拍照示范引导，挂「拍照示范」角标（与建档/穿搭空态示范图同一政策，标识的是「这不是你的照片」，不是 AI 标识）。
